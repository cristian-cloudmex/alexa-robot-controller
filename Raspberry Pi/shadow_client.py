...
from robot_movements import RobotMoves
...
class RobotControllerMQTTClient():
    """
    ...
    def _subscribe_update_callback(self, client, userdata, message):
        ...
        try:
            ...
            robot_data = payload_dict['current']['state']['desired']
            if self.robotMoves.needs_updating(robot_data):
                self.robotMoves.update_robot_data(robot_data)
                reported_payload = {
                                       'state': {
                                           'reported': self.robotMoves.current_setting()
                                       }
                                   }
    ...
    def run_app(self, set_desired_state=False):
        ...
        start_payload = {
                            'state': {
                                'reported': self.robotMoves.current_setting(),
                                'desired': self.robotMoves.current_setting()
                            }
                        }
...
def main():
    ...
    Args:
        None
    """
    alexarobotcontroller_mqtt_client = RobotControllerMQTTClient()
    alexarobotcontroller_mqtt_client.run_app()
